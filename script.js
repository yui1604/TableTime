...
function updateTheme() {
  ...
  const today = now.getDate(); // 今日の日付（日）を取得
  ...
  // 2. カレンダー更新ロジック
  // 最後に更新した日と今日の日付が異なる場合、カレンダーを再生成する
  // (初回実行時は lastUpdatedDay が null なので必ず実行される)
  if (lastUpdatedDay !== today) {
    generateCalendar(); // カレンダーを再生成
    lastUpdatedDay = today; // 最後に更新した日付を今日に設定
  }
}
...
// 初回読み込み時にテーマを設定（ここでカレンダーも初期生成されます）
updateTheme();

// 1秒ごとに時間をチェックしてテーマとカレンダーの更新を処理
